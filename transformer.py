...
#     transformer_block = TransformerBlock(embed_dim=64, num_heads=3, ff_dim=256, rate=0.3)
#     x = transformer_block(x, training=True)
    
deep_set_output_dim = int(416 * 0.5)
agg_block = DeepSet(input_dim = 32, hidden_dim = 416, output_dim = deep_set_output_dim, num_encode = 1, num_decode = 3)
x = agg_block(x) 
    
# # Flatten the transformer output for input into the MLP
# x = Flatten()(x)
# x = Dense(320, activation='relu', kernel_regularizer=l2(0.001), name='mlp_dense_1')(x)
